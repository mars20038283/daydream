...
public class EmaLoader_5min {
	...
	static {
		...
		try {
			list12.add(get5MinDate(startDate).getShoupan());
			list26.add(get5MinDate(startDate).getShoupan());
			listShoupan.add((int) (get5MinDate(startDate).getShoupan()));
			listDea.add(0D);
			listMacd.add(0D);
	...
	public static void main(String[] args) {
		try {
			Date tmpDate = DateUtils.addMinute(goods, startDate, 5);
			while (tmpDate != null && !tmpDate.after(endDate)) {
				try {
					...
					try {
						tmpDate = DateUtils.addMinute(goods, tmpDate, 5);
					} catch (Exception e) {
						break;
					}
				}
